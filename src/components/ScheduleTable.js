import React from 'react';

class ScheduleTable extends React.Component {
    render() {
        return (
            <table>
              <thead>
                  <tr>
                    <th>Workshop #</th>
                    <th>Date</th>
                    <th>Topic</th>
                    <th>Link to Workshop Notebook + Recording</th>
                    <th>Milestone Deadline + Link to Deliverable</th>
                  </tr>
              </thead>
              <tbody>
                  <tr>
                    <td><text style={{ color: 'red' }}>1</text></td>
                    <td><text style={{ color: 'red' }}>September 13th 2021 6pm - 7pm EDT</text></td>
                    <td><text style={{ color: 'red' }}>Introduction to Python + Intro to Bootcamp Project</text></td>
                    <td><text style={{ color: 'red' }}>Stay Tuned!</text></td>
                    <td><text style={{ color: 'red' }}>September 26th, 2021 11:59pm EDT</text></td>
                  </tr>
                  <tr>
                    <td><text style={{ color: 'red' }}>2</text></td>
                      <td><text style={{ color: 'red' }}>September 27th 2021 6pm - 7pm EDT</text></td>
                      <td><text style={{ color: 'red' }}>Introduction to Pandas + Exploratory Data Analysis</text></td>
                      <td><text style={{ color: 'red' }}>Stay Tuned!</text></td>
                      <td><text style={{ color: 'red' }}>October 10th, 2021 11:59pm EDT</text></td>
                  </tr>
                  <tr>
                    <td><text style={{ color: 'red' }}>3</text></td>
                    <td><text style={{ color: 'red' }}>October 11th, 2021 6pm - 7pm EDT</text></td>
                    <td><text style={{ color: 'red' }}>Introductory Data Preprocessing</text></td>
                      <td><text style={{ color: 'red' }}>Stay Tuned!</text></td>
                    <td><text style={{ color: 'red' }}>October 24th, 2021 11:59pm EDT</text></td>
                  </tr>
                  <tr>
                      <td><text style={{ color: 'red' }}>4</text></td>
                      <td><text style={{ color: 'red' }}>October 25th, 2021 6pm - 7pm EDT</text></td>
                      <td><text style={{ color: 'red' }}>Introduction to Machine Learning</text></td>
                      <td><text style={{ color: 'red' }}>Stay Tuned!</text></td>
                      <td><text style={{ color: 'red' }}>November 7th, 2021 11:59pm EDT</text></td>
                  </tr>
                  <tr>
                      <td><text style={{ color: 'red' }}>5</text></td>
                      <td><text style={{ color: 'red' }}>November 8th, 2021 6pm - 7pm EDT</text></td>
                      <td><text style={{ color: 'red' }}>TBD</text></td>
                      <td><text style={{ color: 'red' }}>Stay Tuned!</text></td>
                      <td><text style={{ color: 'red' }}>November 21st, 2021 11:59pm EDT</text></td>
                  </tr>
              </tbody>
            </table>

        );
    }
}

export default ScheduleTable;
